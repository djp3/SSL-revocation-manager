...
/**
 * <p>This is used to check if an SSL connection has a known good server side
 * certificate that can be verified.  If it is it is an accepted connection,
 * otherwise it is rejected.</p>
 * 
 *  <p>Currently this class just checks for the presence of properly signed CT log
 *  promises embedded in the SSL certificate.  It does not request the proof from the CT
 *  authority that it was incorporated into the transparency log.
 *  Hopefully someone will volunteer to add that capacity
 *  in the future. </p>
 *
 ...
 * </ul>
 *
 *  This only validates using the first approach.
 *  Reference this base code:
 *  	https://github.com/google/certificate-transparency-java
 *  and this pull request on it
...
public class CTVerifier extends Verifier<BigInteger, VerificationStatus> {
	...
	/**
	 * A CT log's Id is created by using this hash algorithm on the CT log public
	 * key
	 */
	...
	public CTVerifier(Configuration config) throws FileNotFoundException, ClassNotFoundException, IOException, InvalidKeySpecException, NoSuchAlgorithmException {
    	super(config); 
	 ...
	 *             the CT log key isn't RSA or EC, the key is probably corrupt.
	 * @throws NoSuchAlgorithmException
	 *             the crypto provider couldn't supply the hashing algorithm or the
	 *             key algorithm. This probably means you are using an ancient or
	 *             bad crypto provider.
	 */
	private void buildLogSignatureVerifiers() throws InvalidKeySpecException, NoSuchAlgorithmException {
		...
		try {
			...
			for (Ct.SignedCertificateTimestamp sct : sctsInCertificate) {
				...
				if (verifiers.containsKey(logId)) {
					getLog().info("SCT trusted log " + logId);
	 ...
	 *            The certificate we want to check if revoked.
	 * @param issuerCert
	 *            Not needed for a Certificate Transparency check
	 * @return revocation status of the peer certificate.
	 * @throws CertificateVerificationException
	 *
	 */
